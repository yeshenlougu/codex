...
import { Card, Button, Typography, Row, Col, Tag, Empty, Modal, Select, TimePicker, Input, message, Popconfirm } from 'antd';
import {
...
export default function ScheduledPage() {
  ...
  const load = useCallback(() => {
    setLoading(true);
    listSchedules().then(s => setSchedules(s.schedules || [])).catch(() => {})
  ...
  const handleCreate = async () => {
    ...
    try {
      await createSchedule({
        ...
        prompt: taskName,
        cron_expr: cronExpr,
        category: selectedTemplate ? 'daily' : 'daily',
      });
      message.success(`已创建定时任务: ${taskName}`);
      setCreateOpen(false); setTaskName(''); setSelectedTemplate('');
      load();
    } catch (e: any) { message.error(e.message); }
  };

  const handleDelete = async (id: string) => {
    try { await deleteSchedule(id); message.success('已删除'); load(); }
    catch (e: any) { message.error(e.message); }
  };

  const cronLabel = (expr: string) => {
    const m: Record<string, string> = { daily: '每日', weekdays: '工作日', weekly: '每周' };
    return m[expr] || expr;
  };
    ...
    <div style={{ height: '100%', overflowY: 'auto', background: 'var(--bg-root)' }}>
      <div style={{ maxWidth: 720, margin: '0 auto', padding: '32px 24px' }}>
        ...
        <div style={{ marginTop: 32 }}>
          ...
          {loading ? (
            <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description={<Text type="secondary" style={{ fontSize: 12 }}>加载中...</Text>} />
          ) : schedules.length === 0 ? (
            <Empty
            ...
            <Row gutter={[12, 12]}>
              {schedules.map(sch => (
                <Col span={24} key={sch.id}>
                  <Card size="small">
                    <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                      <div style={{ fontSize: 22, flexShrink: 0 }}>{sch.category === 'monitor' ? '🔔' : sch.category === 'weekly' ? '📊' : '📅'}</div>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                          <Text strong style={{ fontSize: 13 }}>{sch.name}</Text>
                          <Tag color={sch.enabled ? 'green' : 'default'} style={{ margin: 0, fontSize: 10, lineHeight: '18px' }}>
                            {sch.enabled ? '运行中' : '已暂停'}
                          </Tag>
                        </div>
                        <Text type="secondary" style={{ fontSize: 11, display: 'block' }}>
                          <ClockCircleOutlined style={{ marginRight: 4 }} />{cronLabel(sch.cron_expr)}
                        </Text>
                        {sch.last_run && <Text type="secondary" style={{ fontSize: 10, color: 'var(--text-muted)' }}>上次: {new Date(sch.last_run).toLocaleString()}</Text>}
                      </div>
                      <Button type="text" size="small" icon={<ReloadOutlined />} onClick={load} />
                      <Popconfirm title="删除此任务?" onConfirm={() => handleDelete(sch.id)}>
